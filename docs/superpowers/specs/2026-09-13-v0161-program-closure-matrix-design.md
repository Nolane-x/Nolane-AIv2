...
Base authority for this matrix is `main@803fb474eca9cf57713f190e89ef17a113f335e5` plus preserved GitHub scientific evidence from closed/merged experiment PRs. The frozen Stage-A protocol digest remains `c010d90b9d626cfde4f7727b76fcd053f1ebf7f2f7aa201d7d13d2d828cef440`.
...
Consequence: EXP-287 is authorized as the immediate learned-conflict-localization court. EXP-288 remains blocked until learned localization itself establishes sufficient value; oracle value alone cannot authorize learned backjump behavior.
...
Consequence: EXP-290 is authorized as a learned-clause transfer/generalization research seam. It need not wait for EXP-287 because its direct parent is the already promoted local-nogood result, but the program currently prioritizes EXP-287 to resolve the earlier conflict-localization dependency first.
...
Consequence: EXP-298 is authorized as a cross-domain transfer court, but no open-domain semantic claim may be inherited from EXP-297.
...
| EXP-286 | Oracle conflict-core value | `SCIENTIFICALLY_CLOSED_PROMOTE` | Authorizes EXP-287 learned localization |
| EXP-287 | Learned conflict localization approaches oracle value | `AUTHORIZED_NEXT_STAGE` | Highest-priority next experiment |
| EXP-288 | Dependency-directed backjump vs chronological rollback | `BLOCKED_ON_PARENT` | Requires EXP-287 learned-localization gate |
| EXP-289 | Episode-local nogoods | `SCIENTIFICALLY_CLOSED_PROMOTE` | Authorizes EXP-290 |
| EXP-290 | Learned clauses transfer across surface randomization | `AUTHORIZED_NEXT_STAGE` | Can open after separate preregistration; secondary priority |
...
| EXP-298 | Transfer beyond SAT/CSP | `AUTHORIZED_NEXT_STAGE` | Must preserve EXP-297 semantic-authority limits |
...
The next active implementation is EXP-287 because:

1. EXP-286 produced real EV-E3 positive oracle value, so there is demonstrated headroom to recover.
2. The learned-versus-oracle gap is now the decisive uncertainty.
3. EXP-288 is causally downstream of learned localization; running backjump first would confound localization with control.
4. EXP-290 and EXP-298 are independently authorized but can proceed later without blocking EXP-287.
5. EXP-279 V10 is explicitly not to be tuned or rerun; continuing there immediately would violate its closure disposition.

## Closure discipline

A program row changes state only with an immutable evidence record that names the experiment, frozen hypothesis, evidence level, scientific/development decision, exact code/protocol identity, and scope boundary. Negative results reduce the active architecture. They are not converted into “partial success” merely to keep a roadmap moving.

A downstream experiment may be skipped permanently if its parent is killed and no scientifically distinct replacement hypothesis is approved. Completion of V0.16.1 therefore means closing the dependency graph honestly, not mechanically executing all 24 IDs.
