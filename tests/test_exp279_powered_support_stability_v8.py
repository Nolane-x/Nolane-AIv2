...
def _fold(*, episodes: int = 8, rescues: int = 1) -> dict[str, int]:
    ...
    return {
        ...
        "both_failure": episodes - rescues - both_success,
    }
...
def test_v8_power_contract_is_frozen_to_global_96_fold_family() -> None:
    ...
    q_zero = (1.0 - FROZEN_V7_FLOOR) ** required_fold_episodes()
    assert q_zero * FROZEN_GLOBAL_FOLDS < FROZEN_FAMILY_ALPHA
    assert (1.0 - FROZEN_V7_FLOOR) ** (required_fold_episodes() - 8) * FROZEN_GLOBAL_FOLDS > FROZEN_FAMILY_ALPHA


def test_v8_root_namespaces_are_fresh_frozen_and_cross_budget_disjoint() -> None:
    assert ROOT_PREFIX == "20260912-exp279-powered-support-v8-dev"
    assert TRAIN_BUDGETS == (60, 120)
    assert CANONICAL_INDICES == (0, 1, 2, 3)
    assert PROBE_INDICES == (0, 1, 2, 3)
    assert canonical_root(60, 2) == "20260912-exp279-powered-support-v8-dev::train::60::canonical::2"
    assert probe_root(120, 3, 1) == "20260912-exp279-powered-support-v8-dev::train::120::probe::3::1"
    ...
    for root in flat60 | flat120:
        assert "multiroot-support-v7" not in root
...
def test_v8_root_helpers_reject_unregistered_identifiers(fn, args) -> None:
    with pytest.raises(ValueError):
        fn(*args)


def test_v8_diagnostic_fold_assignment_gives_444_replicates_per_fold() -> None:
    counts = [0, 0, 0]
    for replicate in range(PROBE_REPLICATES):
        counts[diagnostic_fold(replicate)] += 1
    assert counts == [444, 444, 444]
    assert [value * 8 for value in counts] == [3552, 3552, 3552]
...
def test_v8_one_sided_wilson_lower_bound_matches_frozen_formula() -> None:
    ...
    assert wilson_lower_bound(23, 6336) == pytest.approx(0.0025814897747382503)
...
def test_v8_probe_and_canonical_support_summaries_close_counts() -> None:
...
def test_v8_train_cell_classification_priority_is_frozen() -> None:
    recurrent = [_canonical() for _ in range(4)]
    assert classify_train_cell(recurrent) == "SUPPORT_RECURRENT"

    intermittent = list(recurrent)
    intermittent[2] = _canonical(
        probe_fold_rescues=((1, 1, 1), (1, 0, 1), (1, 1, 1), (1, 1, 1))
    )
    assert classify_train_cell(intermittent) == "PROBE_SUPPORT_INTERMITTENT"
    ...
    assert classify_train_cell(collapse) == "MODEL_ROOT_SUPPORT_COLLAPSE"


def test_v8_support_summaries_reject_nonclosing_partitions() -> None:
    bad = _fold()
    bad["both_failure"] += 1
...
def test_v8_tiny_shard_is_augmentation_only_and_embeds_exact_provenance() -> None:
    receipt = run_exp279_powered_support_stability_v8_shard(
        ...
        code_digest="c" * 64,
        scientific_branch_head=HEX_A,
        executed_commit=HEX_B,
    )
    ...
    assert receipt["scientific_branch_head"] == HEX_A
    assert receipt["executed_commit"] == HEX_B
    ...
    for forbidden in ("stop_logits", "branch_logits", "raw_targets", "predictions", "route_scores"):
        assert forbidden not in encoded


def test_v8_scientific_validator_rejects_wrong_identity_and_boundaries() -> None:
    receipt = run_exp279_powered_support_stability_v8_shard(
        train_replicates=60,
        canonical_index=0,
        d_model=8,
    ...
    validate_v8_shard(receipt, require_frozen_scientific=False)

    for key, bad_value in [
        ("scientific_branch_head", "x" * 40),
        ("executed_commit", "y" * 40),
        ("evaluation_rng_stream_used", True),
        ("evaluation_targets_used", True),
        ...
        with pytest.raises(ValueError):
            validate_v8_shard(mutated, require_frozen_scientific=False)


def test_v8_powered_episode_totals_are_exact() -> None:
    per_fold = required_fold_episodes()
    per_probe = per_fold * PROBE_FOLDS
    per_canonical = per_probe * 4
    per_budget = per_canonical * 4
    assert per_fold == 3552
    assert per_probe == 10656
    assert per_canonical == 42624
    assert per_budget == 170496
    assert per_budget * 2 == 340992
    assert PROBE_REPLICATES * 8 == per_probe
    assert math.isclose(FROZEN_V7_FLOOR, 23 / 6336, rel_tol=0.0, abs_tol=0.0011)
