...
def _base_command(output: Path) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "scripts" / "run_exp279_prebranch_observability_v4_dev.py"),
        "--tiny",
        "--train-replicates", "3",
        "--probe-replicates", "9",
        "--probe-folds", "3",
...
def test_v4_cli_writes_augmentation_only_prebranch_artifact(tmp_path: Path) -> None:
    output = tmp_path / "prebranch-v4.json"
    ...
    assert artifact["schema"] == "NLM-EXP-279-PREBRANCH-OBSERVABILITY-COURT-V4"
    ...
    assert artifact["feature_boundary"]["branch_gru_used_for_probe_features"] is False
    assert artifact["probe_economics"]["probe_is_deployable"] is False
    assert artifact["probe_economics"]["probe_inference_flops_charged_to_primary_utility"] is False
    assert artifact["probe_economics"]["successor_must_account_selector_flops"] is True
    ...
    assert stdout["fresh_evaluation_lineage_consumed"] is False
    assert stdout["confirmatory_data_consumed"] is False


def test_v4_cli_refuses_existing_output_before_protocol_work(tmp_path: Path) -> None:
    output = tmp_path / "prebranch-v4.json"
...
def test_v4_cli_rejects_noncanonical_protocol_even_with_matching_digest(tmp_path: Path) -> None:
    output = tmp_path / "prebranch-v4.json"
