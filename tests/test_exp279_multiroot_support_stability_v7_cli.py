...
SHARD_SCRIPT = ROOT / "scripts" / "run_exp279_multiroot_support_stability_v7_dev.py"
AGGREGATE_SCRIPT = ROOT / "scripts" / "aggregate_exp279_multiroot_support_stability_v7_dev.py"
...
def test_v7_shard_cli_writes_augmentation_only_receipt(tmp_path: Path) -> None:
    output = tmp_path / "v7-shard.json"
    completed = subprocess.run(
        _shard_command(output),
        cwd=ROOT,
        check=True,
        text=True,
        capture_output=True,
    ...
    assert stdout["train_replicates"] == 60
    assert stdout["canonical_index"] == 0


def test_v7_shard_cli_refuses_existing_output_before_protocol_work(tmp_path: Path) -> None:
    output = tmp_path / "v7-shard.json"
...
def test_v7_shard_cli_rejects_noncanonical_protocol_even_with_matching_digest(
    tmp_path: Path,
) -> None:
    output = tmp_path / "v7-shard.json"
...
def test_v7_shard_cli_exposes_no_evaluation_or_lineage_knobs() -> None:
    completed = subprocess.run(
...
def test_v7_budget_cli_refuses_existing_output_before_loading_shards(tmp_path: Path) -> None:
    output = tmp_path / "v7-budget.json"
    output.write_text("sentinel\n", encoding="utf-8")
