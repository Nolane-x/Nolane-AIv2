...
SCRIPT = ROOT / "scripts" / "run_exp279_counterfactual_delta_distill_v6_dev.py"
...
def test_v6_cli_writes_cdd_augmentation_only_artifact(tmp_path: Path) -> None:
    output = tmp_path / "cdd-v6.json"
...
def test_v6_cli_refuses_existing_output_before_protocol_work(tmp_path: Path) -> None:
    output = tmp_path / "cdd-v6.json"
...
def test_v6_cli_rejects_noncanonical_protocol_even_with_matching_digest(tmp_path: Path) -> None:
    output = tmp_path / "cdd-v6.json"
