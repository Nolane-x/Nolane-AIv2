...
def _base_command(output: Path) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "scripts" / "run_exp279_augmentation_separability_v2_dev.py"),
        "--tiny",
        "--train-replicates", "3",
        "--probe-replicates", "9",
        "--probe-folds", "3",
...
def test_v2_cli_writes_augmentation_only_fold_local_court_artifact(tmp_path: Path) -> None:
    output = tmp_path / "separability-v2.json"
    ...
    assert artifact["schema"] == "NLM-EXP-279-AUGMENTATION-SEPARABILITY-COURT-V2"
...
def test_v2_cli_refuses_existing_output_before_protocol_work(tmp_path: Path) -> None:
    output = tmp_path / "separability-v2.json"
...
def test_v2_cli_rejects_noncanonical_protocol_even_with_matching_digest(tmp_path: Path) -> None:
    output = tmp_path / "separability-v2.json"
