...
from nolane_ai.experiments.exp279_augmentation_direct_utility_v3 import PROBE_KINDS
from nolane_ai.experiments.exp279_augmentation_direct_utility_v3_cross_cell import (
    classify_exp279_direct_utility_v3_cross_cell,
)


def _cell(*, support: bool = True, linear: bool = False, mlp: bool = False, deepsets: bool = False) -> dict:
    passes = {"LINEAR_MEAN": linear, "MLP_MEAN": mlp, "DEEPSETS": deepsets}
    return {
...
def test_cross_cell_requires_same_linear_family_at_60_and_120() -> None:
    result = classify_exp279_direct_utility_v3_cross_cell({60: _cell(linear=True), 120: _cell(linear=True)})
    assert result["decision"] == "ROBUST_LINEAR_DIRECT_UTILITY"
...
def test_cross_cell_requires_same_rich_family_not_mixed_cherry_pick() -> None:
    robust = classify_exp279_direct_utility_v3_cross_cell({60: _cell(mlp=True), 120: _cell(mlp=True)})
    mixed = classify_exp279_direct_utility_v3_cross_cell({60: _cell(mlp=True), 120: _cell(deepsets=True)})
    assert robust["decision"] == "ROBUST_RICH_DIRECT_UTILITY"
    assert robust["robust_probe_families"] == ["MLP_MEAN"]
    assert robust["successor_design_authorized"] is True
    assert mixed["decision"] == "NO_ROBUST_DIRECT_UTILITY"
...
def test_cross_cell_support_failure_is_inconclusive_and_fail_closed() -> None:
    result = classify_exp279_direct_utility_v3_cross_cell({60: _cell(support=False, linear=True), 120: _cell(linear=True)})
...
def test_cross_cell_rejects_missing_decision_cell() -> None:
    with pytest.raises(ValueError, match="train 60 and train 120"):
        classify_exp279_direct_utility_v3_cross_cell({60: _cell()})


def test_cross_cell_rejects_evaluation_boundary_violation() -> None:
    bad = _cell()
    bad["data_boundary"]["evaluation_targets_used"] = True
    with pytest.raises(ValueError, match="evaluation boundary"):
        classify_exp279_direct_utility_v3_cross_cell({60: bad, 120: _cell()})
