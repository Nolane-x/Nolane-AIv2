...
def _base_command(output: Path) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "scripts" / "run_exp279_augmentation_direct_utility_v3_dev.py"),
        "--tiny",
        "--train-replicates", "3",
        "--probe-replicates", "9",
        "--probe-folds", "3",
...
def test_v3_cli_writes_augmentation_only_direct_utility_artifact(tmp_path: Path) -> None:
    output = tmp_path / "direct-utility-v3.json"
    completed = subprocess.run(_base_command(output), cwd=ROOT, check=True, text=True, capture_output=True)
    artifact = json.loads(output.read_text(encoding="utf-8"))
    stdout = json.loads(completed.stdout.strip())

    assert artifact["schema"] == "NLM-EXP-279-AUGMENTATION-DIRECT-UTILITY-COURT-V3"
...
def test_v3_cli_refuses_existing_output_before_protocol_work(tmp_path: Path) -> None:
    output = tmp_path / "direct-utility-v3.json"
...
def test_v3_cli_rejects_noncanonical_protocol_even_with_matching_digest(tmp_path: Path) -> None:
    output = tmp_path / "direct-utility-v3.json"
    ...
    command = _base_command(output)
    command.extend(["--protocol", str(protocol), "--protocol-digest-file", str(digest)])
    completed = subprocess.run(command, cwd=ROOT, check=False, text=True, capture_output=True)
    assert completed.returncode != 0
