...
def _base_command(output: Path, registry: Path) -> list[str]:
    return [
        sys.executable,
        str(ROOT / "scripts" / "run_exp279_paired_dev.py"),
        "--tiny",
        "--train-replicates", "3",
        "--eval-replicates", "3",
        "--batch-size", "2",
...
def test_exp279_paired_cli_writes_development_and_registry_artifacts(tmp_path: Path) -> None:
    execution_path = tmp_path / "exp279.json"
    ...
    assert execution["schema"] == "NLM-EXP-279-PAIRED-DEV-EVAL-V1"
    ...
    assert exp279["paired_execution_evidence"]["artifact_digest"] == execution["artifact_digest"]
...
def test_exp279_paired_cli_refuses_existing_output_before_any_execution(tmp_path: Path) -> None:
    execution_path = tmp_path / "exp279.json"
    registry_path = tmp_path / "registry.json"
...
def test_exp279_paired_cli_rejects_noncanonical_protocol_even_with_matching_digest(tmp_path: Path) -> None:
    execution_path = tmp_path / "exp279.json"
    registry_path = tmp_path / "registry.json"
