...
WORKFLOW = ROOT / ".github" / "workflows" / "exp279-real-gate-b-ceremony.yml"
ARM_PATH = ".github/ceremony/exp279-real-gate-b-arm.json"
PROTOCOL_SHA = "c010d90b9d626cfde4f7727b76fcd053f1ebf7f2f7aa201d7d13d2d828cef440"
CEREMONY_BRANCH = "exp279-real-gate-b-ceremony"


def _workflow_text() -> str:
    assert WORKFLOW.is_file(), f"missing dormant real EXP-279 ceremony workflow: {WORKFLOW}"
    return WORKFLOW.read_text(encoding="utf-8")


def test_exp279_real_gate_b_workflow_exists_and_is_arm_only() -> None:
    text = _workflow_text()
    assert f"branches: [{CEREMONY_BRANCH}]" in text
...
def test_exp279_real_gate_b_arm_commit_is_only_orchestration_marker() -> None:
    text = _workflow_text()
    assert "NLM-EXP-279-REAL-GATE-B-ARM-V1" in text
...
def test_exp279_real_gate_b_requires_baseline_normal_and_focused_ci() -> None:
    text = _workflow_text()
    assert "actions/workflows/ci.yml/runs" in text
    assert "actions/workflows/exp279-confirmatory-ci.yml/runs" in text
...
def test_exp279_real_gate_b_uses_fixed_non_tiny_routing_geometry() -> None:
    text = _workflow_text()
    for token in (
    ...
    assert "build_exp279_test_only_development.py" not in text
    assert "test_only_fixture" not in text


def test_exp279_real_gate_b_preflight_is_outcome_agnostic_before_beacon() -> None:
    text = _workflow_text()
    for token in (
        "CONFIRMATORY_GATE_A_PREPARED",
        "NOT_READY_SIMPLE_BLOCKED_MEAN_NONPOSITIVE",
        "NOT_READY_VARIANCE_EXCEEDS_MAX_N",
        "confirmatory_n",
        "unclamped_required_n",
        "confirmatory_data_consumed",
        "challenge_materialized",
    ):
        assert token in text
    assert "Stage authoritative NOT_READY result" in text


def test_exp279_real_gate_a_precedes_future_public_beacon_and_uses_runtime_seal_time() -> None:
    text = _workflow_text()
    gate_a = text.index("python scripts/prepare_exp279_confirmatory_gate_a.py")
...
def test_exp279_real_gate_a_normalizes_git_commit_time_to_utc_before_sealing() -> None:
    text = _workflow_text()
...
def test_exp279_real_gate_b_writes_inference_marker_before_scientific_cli() -> None:
    text = _workflow_text()
    marker = "exp279-real-gate-b-inference-started-${{ github.sha }}"
    assert marker in text
    marker_index = text.index(marker)
    gate_b_index = text.index("python scripts/run_exp279_confirmatory_gate_b.py")
...
def test_exp279_real_gate_b_preserves_all_scientific_outcomes_and_blocked_holm_evidence() -> None:
    text = _workflow_text()
    for token in (
        "INVALID_RUN",
...
def test_exp279_real_gate_b_rejects_replay_after_inference_or_outcome_artifact() -> None:
    text = _workflow_text()
    assert "actions/artifacts?name=" in text
    assert "exp279-real-gate-b-inference-started-${{ github.sha }}" in text
    assert "exp279-real-gate-b-outcome-${{ github.sha }}" in text
    assert "authoritative EXP-279" in text
