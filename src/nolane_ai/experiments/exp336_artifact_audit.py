...
def _normalize_digest(value: str) -> str:
    ...
    if len(raw) != 64:
        raise ValueError("EXP-336 audit digest length")
    try:
        int(raw, 16)
    except ValueError as exc:
        raise ValueError("EXP-336 audit digest hex") from exc
    return raw.lower()
...
def _json_member(archive: ZipFile, name: str) -> dict[str, Any]:
    try:
        ...
        raise ValueError(f"EXP-336 audit missing ZIP member: {name}") from exc
    value = json.loads(raw.decode("utf-8"))
    if not isinstance(value, dict):
        raise ValueError(f"EXP-336 audit JSON object required: {name}")
    return value
...
def _identity_fields(identity: Mapping[str, Any]) -> tuple[str, str]:
    try:
        materialized = Exp336ExecutionIdentity(**dict(identity))
        validate_execution_identity(materialized)
    except (TypeError, ValueError) as exc:
        raise ValueError("EXP-336 audit execution identity") from exc
    return (
        _normalize_digest(materialized.source_tree_digest),
        _normalize_digest(materialized.exp336_execution_digest),
    )
...
def audit_chunk_zip(
    ...
    *,
    ...
    if not 0 <= expected_chunk_index < CHUNK_COUNT:
        raise ValueError("EXP-336 audit chunk index")
    ...
    with ZipFile(artifact_path, "r") as archive:
        ...
        if summary.get("cumulative_exposure_per_world") != expected_exposure:
            raise ValueError("EXP-336 audit summary exposure")
        if summary.get("cumulative_source_updates") != expected_updates:
            raise ValueError("EXP-336 audit summary updates")
        if summary.get("cumulative_training_step") != expected_step:
            raise ValueError("EXP-336 audit summary cumulative step")

        for key, expected in AUTHORIZATION_FLAGS.items():
            if summary.get(key) is not expected:
                raise ValueError(f"EXP-336 audit authorization drift: {key}")
        ...
        if not isinstance(claimed_bundle, str) or canonical_digest(materialized) != claimed_bundle:
            raise ValueError("EXP-336 audit bundle digest")

        arms = summary.get("arms")
        if not isinstance(arms, Mapping) or set(arms) != set(ARMS):
            raise ValueError("EXP-336 audit arm coverage")
        ...
        for arm in ARMS:
            ...
            if files.get("checkpoint") != checkpoint_name or files.get("receipt") != receipt_name:
                raise ValueError("EXP-336 audit file manifest")
            ...
            if not isinstance(claimed_receipt, str) or _receipt_digest(receipt) != claimed_receipt:
                raise ValueError("EXP-336 audit receipt digest")
            if files.get("receipt_digest") != claimed_receipt:
                raise ValueError("EXP-336 audit receipt manifest digest")
            receipt_digests[arm] = claimed_receipt

            for key, expected in AUTHORIZATION_FLAGS.items():
                if receipt.get(key) is not expected:
                    raise ValueError(f"EXP-336 audit receipt authorization drift: {key}")

        if summary.get("control_sham_exact") is not True:
            raise ValueError("EXP-336 audit summary CONTROL/SHAM flag")
        if not sham_equivalent(
            boundaries["CONTROL_CNRS_FULL32"],
            boundaries["SHAM_MEASURE_CNRS_FULL32"],
        ):
            raise ValueError("EXP-336 audit CONTROL/SHAM mismatch")
    ...
    return {
        ...
        "projection_update_count": boundaries["SUBSPACE_PROJECT_CNRS_FULL32"].projection_update_count,
        "projected_target_count": boundaries["SUBSPACE_PROJECT_CNRS_FULL32"].projected_target_count,
...
def audit_chunk_chain(
    ...
    *,
    ...
    if not zip_paths:
        raise ValueError("EXP-336 audit requires at least one chunk artifact")
    if len(zip_paths) > CHUNK_COUNT:
        raise ValueError("EXP-336 audit too many chunks")
    ...
    return {
        "schema": "EXP336-INDEPENDENT-ARTIFACT-CHAIN-AUDIT-V1",
        "verified_chunk_count": len(reports),
        "complete_chunk_chain": len(reports) == CHUNK_COUNT,
        "first_parent_cnrs_zip_digest": PARENT_CNRS_ZIP_DIGEST,
    ...
    *,
    ...
    for key, expected in fixed_parent.items():
        if payload.get(key) != expected:
            raise ValueError(f"EXP-336 final audit parent/prereg drift: {key}")
    ...
    if payload.get("decision") != decision:
        raise ValueError("EXP-336 final audit reducer decision")
    for key, expected in vectors.items():
        if payload.get(key) != expected:
            raise ValueError(f"EXP-336 final audit reducer vector: {key}")

    for key, expected in AUTHORIZATION_FLAGS.items():
        if payload.get(key) is not expected:
            raise ValueError(f"EXP-336 final audit authorization drift: {key}")

    materialized = dict(payload)
    claimed = materialized.pop("evidence_digest", None)
    if not isinstance(claimed, str) or canonical_digest(materialized) != claimed:
        raise ValueError("EXP-336 final audit evidence digest")

    return {
        "schema": "EXP336-INDEPENDENT-FINAL-AUDIT-V1",
