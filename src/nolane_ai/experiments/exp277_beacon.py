...
def build_test_beacon_receipt(
    *,
    ...
    errors = validate_exp277_beacon_receipt(payload)
    if errors:
        raise ValueError("invalid synthetic EXP-277 beacon receipt: " + "; ".join(errors))
    return payload


def validate_exp277_beacon_receipt(
    payload: dict[str, Any],
    *,
    ...
    if not isinstance(payload, dict):
        return ["EXP-277 beacon receipt must be an object"]
    if payload.get("schema") != SCHEMA or payload.get("experiment_id") != EXPERIMENT_ID:
        errors.append("invalid EXP-277 beacon receipt identity")

    for field in ("source", "beacon_id", "evidence_reference"):
        if not isinstance(payload.get(field), str) or not payload.get(field):
            errors.append(f"EXP-277 beacon {field} missing")

    try:
        published = _parse_utc(payload.get("published_at_utc"))
    except ValueError:
        published = None
        errors.append("EXP-277 beacon publication timestamp invalid or not UTC")

    if not _valid_entropy_hex(payload.get("entropy_hex")):
        errors.append("EXP-277 beacon entropy must contain at least 256 bits of hex entropy")
    ...
    if not isinstance(test_only, bool) or not isinstance(scientific, bool):
        errors.append("EXP-277 beacon evidence classification invalid")
...
def derive_exp277_challenge_seed(
    *,
    ...
    if errors:
        raise ValueError("invalid EXP-277 beacon receipt: " + "; ".join(errors))

    material = f"{protocol_digest}|{beacon_receipt['receipt_digest']}|{EXPERIMENT_ID}|{stream}|{replicate}"
