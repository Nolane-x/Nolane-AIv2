...
FINAL_EVIDENCE_SCHEMA = "EXP322-FINAL-EVIDENCE-V1"
...
def parent_authority_payload() -> dict[str, Any]:
    return {
        "exp321_run_id": PARENT_EXP321_RUN_ID,
        "exp321_artifact_id": PARENT_EXP321_ARTIFACT_ID,
        "exp321_evidence_digest": PARENT_EXP321_EVIDENCE_DIGEST,
        "checkpoint_artifact_id": PARENT_CHECKPOINT_ARTIFACT_ID,
        "checkpoint_zip_digest": PARENT_CHECKPOINT_ZIP_DIGEST,
        "receipt_artifact_digest": PARENT_RECEIPT_ARTIFACT_DIGEST,
...
def validate_arm_evidence(payload: Mapping[str, Any]) -> None:
    if payload.get("schema") != ARM_EVIDENCE_SCHEMA:
        raise ValueError("EXP-322 arm evidence schema mismatch")
    arm = payload.get("arm")
    if arm not in ARMS:
        raise ValueError("EXP-322 arm evidence arm mismatch")
    if payload.get("learning_rate") != ARM_LEARNING_RATES[arm]:
        raise ValueError("EXP-322 arm learning-rate mismatch")
...
def build_final_evidence(
    ...
    records = {
        arm: tuple(
            InterventionSnapshot(**item)
            for item in by_arm[arm]["snapshots"]
        )
        for arm in ARMS
    }
