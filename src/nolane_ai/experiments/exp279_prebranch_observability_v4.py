...
PROBE_KINDS = ("STATE_DEEPSETS", "STATE_EVENT_MEAN", "STATE_EVENT_RESIDUAL")
PROBE_ROOT_SUFFIX = "::independent-augmentation-probe-v4"
...
def _fold_direct_utility_metrics(
    ...
    *,
    ...
    if scores.ndim != 1 or stop_exact.ndim != 1 or branch_exact.ndim != 1:
        raise ValueError("V4 fold scores/outcomes must be rank-1")
    if not (scores.shape == stop_exact.shape == branch_exact.shape):
        raise ValueError("V4 fold scores/outcomes must be aligned")
    if scores.numel() == 0:
        raise ValueError("V4 fold must contain at least one episode")
    ...
    if stop_cost <= 0.0 or branch_cost <= stop_cost:
        raise ValueError("V4 direct utility requires 0 < stop FLOPs < branch FLOPs")
    ...
    if actual_solution_delta != expected_solution_delta:
        raise RuntimeError("V4 rescue/harm accounting does not match counterfactual solution delta")
...
def _aggregate_fold_direct_utility_metrics(rows: list[dict[str, Any]]) -> dict[str, Any]:
    if not rows:
        raise ValueError("V4 aggregate requires at least one fold")
    ...
    if stop_flops <= 0.0 or routed_flops <= 0.0:
        raise ValueError("V4 aggregate FLOPs must be positive")
    ...
    if selected != rescue_count:
        raise RuntimeError("V4 aggregate oracle route cardinality drift")
    if routed_solutions - stop_solutions != rescues - harms:
        raise RuntimeError("V4 aggregate solution delta does not match rescue/harm accounting")
...
def _prebranch_features(
    ...
    *,
    ...
    propagation_state = variables + arm._propagate(variables, checked_incidence)
    return {
...
def _state_event_residual_vector(features: dict[str, torch.Tensor]) -> torch.Tensor:
    p_mean = features["p_mean"]
    e_mean = features["e_mean"]
    e_delta = features["e_delta"]
    return torch.cat((p_mean, e_mean, e_delta, p_mean - e_mean, p_mean * e_mean), dim=-1)


class _StateDeepSetsProbe(nn.Module):
...
def _build_probe(kind: str, *, hidden_size: int, seed: int) -> nn.Module:
    if kind not in PROBE_KINDS:
        raise ValueError(f"unknown EXP-279 V4 probe kind: {kind}")
    with torch.random.fork_rng(devices=[]):
        torch.manual_seed(seed)
        if kind == "STATE_DEEPSETS":
            return _StateDeepSetsProbe(hidden_size)
        if kind == "STATE_EVENT_MEAN":
            return _VectorProbe(2 * hidden_size, hidden_size)
        return _VectorProbe(5 * hidden_size, hidden_size)


def _slice_features(features: dict[str, torch.Tensor], mask: torch.Tensor) -> dict[str, torch.Tensor]:
    return {key: value[mask] for key, value in features.items()}


def _probe_input(kind: str, features: dict[str, torch.Tensor]) -> torch.Tensor:
    if kind == "STATE_DEEPSETS":
        return features["propagation_state"]
    if kind == "STATE_EVENT_MEAN":
        return torch.cat((features["p_mean"], features["e_mean"]), dim=-1)
    if kind == "STATE_EVENT_RESIDUAL":
        return _state_event_residual_vector(features)
    raise ValueError(f"unknown EXP-279 V4 probe kind: {kind}")
...
def _probe_inference_flops(kind: str, *, hidden_size: int, variables: int, timesteps: int) -> int:
    h = hidden_size
    p_mean = h * max(variables - 1, 0) + h
    e_mean = h * max(timesteps - 1, 0) + h
    if kind == "STATE_DEEPSETS":
        return variables * (_linear_flops(h, h) + h) + p_mean + _linear_flops(h, h) + h + _linear_flops(h, 1)
    if kind == "STATE_EVENT_MEAN":
        return p_mean + e_mean + _linear_flops(2 * h, h) + h + _linear_flops(h, 1)
    if kind == "STATE_EVENT_RESIDUAL":
        vector_ops = 3 * h
        return p_mean + e_mean + vector_ops + _linear_flops(5 * h, h) + h + _linear_flops(h, 1)
    raise ValueError(f"unknown EXP-279 V4 probe kind: {kind}")
...
def _fit_probe_fold(
    *,
    ...
    train_features = _slice_features(features, train_mask)
    ...
    fit_support_closed = train_counts["positive"] > 0 and train_counts["negative"] > 0

    seed = derive_stream_seed(probe_root_seed, f"EXP-279-PREBRANCH-V4-{kind}", fold, "model_init")
    ...
    if fit_support_closed:
        probe.train()
        probe_train_input = _probe_input(kind, train_features)
        for step in range(probe_steps):
            optimizer.zero_grad(set_to_none=True)
            scores = probe(probe_train_input)
    ...
    probe.eval()
    with torch.no_grad():
        test_scores = probe(_probe_input(kind, _slice_features(features, test_mask)))
    ...
    heldout_support_closed = bool(metrics["support_closed"])
    support_closed = fit_support_closed and heldout_support_closed
    metrics["support_closed"] = support_closed
    metrics["direct_utility_improved"] = bool(
        support_closed and float(metrics["routed_verified_utility"]) > float(metrics["stop_verified_utility"])
    )
    return {
        "fold": fold,
        "folds": folds,
        ...
        "fit_support_closed": fit_support_closed,
        "heldout_support_closed": heldout_support_closed,
...
def _cross_fitted_probe(
    *,
    ...
    seed = derive_stream_seed(probe_root_seed, f"EXP-279-PREBRANCH-V4-{kind}", 0, "model_init")
    parameter_count = sum(parameter.numel() for parameter in _build_probe(kind, hidden_size=hidden_size, seed=seed).parameters())
    return {
        "kind": kind,
        "parameter_count": parameter_count,
        "analytical_probe_flops_per_episode": _probe_inference_flops(
            kind, hidden_size=hidden_size, variables=variables, timesteps=timesteps
        ),
        "probe_flop_semantics": (
            "analytical cheap-feature aggregation plus probe-head FLOPs; event_projection and "
            "propagation_state are already canonical pre-branch computations"
        ),
        "folds": fold_rows,
        "aggregate": aggregate,
...
def _court_classification(probes: dict[str, dict[str, Any]]) -> str:
    ...
    if bool(probes["STATE_DEEPSETS"]["economically_routable"]):
        return "STATE_ONLY_DIRECT_UTILITY_POSITIVE"
    if bool(probes["STATE_EVENT_MEAN"]["economically_routable"]):
        return "EVENT_MEAN_DIRECT_UTILITY_POSITIVE"
    if bool(probes["STATE_EVENT_RESIDUAL"]["economically_routable"]):
        return "EVENT_RESIDUAL_DIRECT_UTILITY_POSITIVE"
    return "NO_DIRECTLY_ECONOMIC_PREBRANCH_SIGNAL"


def validate_exp279_prebranch_observability_v4(payload: dict[str, Any]) -> list[str]:
    errors: list[str] = []
    if payload.get("schema") != SCHEMA:
        errors.append("V4 pre-branch observability schema mismatch")
    if payload.get("evidence_level") != "EV-E2" or payload.get("decision") != "UNVERIFIED":
        errors.append("V4 evidence boundary mismatch")
    if payload.get("scientific_evidence_eligible") is not False:
        errors.append("V4 cannot be scientific-evidence eligible")
    for key in (
        ...
        if payload.get(key) is not False:
            errors.append(f"V4 {key} must remain false")

    boundary = payload.get("data_boundary") or {}
    if boundary.get("training_rng_stream") != "augmentation":
        errors.append("V4 canonical training must remain augmentation-only")
    if boundary.get("probe_rng_stream") != "augmentation":
        errors.append("V4 probe stream must remain augmentation-only")
    if boundary.get("evaluation_rng_stream_used") is not False:
        errors.append("V4 cannot use evaluation RNG stream")
    if boundary.get("evaluation_targets_used") is not False:
        errors.append("V4 cannot use evaluation targets")
    if boundary.get("probe_root_seed") == payload.get("root_seed"):
        errors.append("V4 probe root must be independent from canonical training root")

    feature_boundary = payload.get("feature_boundary") or {}
    if feature_boundary.get("branch_gru_used_for_probe_features") is not False:
        errors.append("V4 probe features cannot execute branch GRU")
    if feature_boundary.get("allowed_temporal_summary") != "last_projected_event_minus_first_projected_event":
        errors.append("V4 temporal feature boundary changed")
    if feature_boundary.get("event_projection_is_canonical_prebranch_shared_compute") is not True:
        errors.append("V4 event projection provenance missing")

    config = payload.get("probe_config") or {}
    if config.get("probe_kinds") != list(PROBE_KINDS):
        errors.append("V4 probe family changed")
    if config.get("folds") != 3:
        errors.append("V4 court is frozen to three folds")
    if config.get("fit_and_heldout_support_required") is not True:
        errors.append("V4 fit/heldout support gate drift")
    if config.get("raw_scores_compared_across_folds") is not False:
        errors.append("V4 cannot compare raw scores across folds")
    if config.get("raw_scores_exported") is not False:
        errors.append("V4 cannot export raw held-out scores")
    if config.get("oracle_route_cardinality_is_deployable") is not False:
        errors.append("V4 oracle route cardinality cannot be deployable")

    rule = payload.get("economic_rule") or {}
    if rule.get("metric") != "direct_counterfactual_verified_solution_per_total_accounted_flop":
        errors.append("V4 direct economic metric changed")
    if rule.get("posterior_break_even_threshold_used") is not False:
        errors.append("V4 cannot use posterior break-even threshold")
    if rule.get("cross_fold_aggregation") != "sum_solution_counts_and_total_accounted_flops":
        errors.append("V4 economic aggregation changed")

    probe_economics = payload.get("probe_economics") or {}
    if probe_economics.get("probe_is_deployable") is not False:
        errors.append("V4 diagnostic probe cannot be deployable")
    if probe_economics.get("probe_inference_flops_charged_to_primary_utility") is not False:
        errors.append("V4 primary utility must not silently charge diagnostic probe FLOPs")
    if probe_economics.get("successor_must_account_selector_flops") is not True:
        errors.append("V4 successor selector FLOP requirement missing")

    probes = payload.get("probes") or {}
    if set(probes) != set(PROBE_KINDS):
        errors.append("V4 probe receipts incomplete")
    else:
        for kind, probe in probes.items():
            if probe.get("raw_scores_compared_across_folds") is not False:
                errors.append(f"V4 {kind} compared raw scores across folds")
            if int(probe.get("parameter_count", 0) or 0) <= 0:
                errors.append(f"V4 {kind} parameter receipt invalid")
            if int(probe.get("analytical_probe_flops_per_episode", 0) or 0) <= 0:
                errors.append(f"V4 {kind} probe FLOP receipt invalid")
            aggregate = probe.get("aggregate") or {}
            if aggregate.get("raw_scores_compared_across_folds") is not False:
                errors.append(f"V4 {kind} aggregate compared raw scores across folds")
            expected_pass = bool(
                aggregate.get("support_closed")
                and float(aggregate.get("routed_verified_utility", 0.0)) > float(aggregate.get("stop_verified_utility", 0.0))
            )
            if probe.get("economically_routable") is not expected_pass:
                errors.append(f"V4 {kind} economic pass receipt mismatch")
            for fold in probe.get("folds") or []:
                if fold.get("raw_scores_exported") is not False:
                    errors.append(f"V4 {kind} exported raw fold scores")
                expected_support = bool(fold.get("fit_support_closed") and fold.get("heldout_support_closed"))
                if fold.get("support_closed") is not expected_support:
                    errors.append(f"V4 {kind} fold support receipt mismatch")

    allowed = {
        "INSUFFICIENT_RESCUE_SUPPORT",
        "STATE_ONLY_DIRECT_UTILITY_POSITIVE",
        "EVENT_MEAN_DIRECT_UTILITY_POSITIVE",
        "EVENT_RESIDUAL_DIRECT_UTILITY_POSITIVE",
        "NO_DIRECTLY_ECONOMIC_PREBRANCH_SIGNAL",
    }
    classification = payload.get("court_classification")
    if classification not in allowed:
        errors.append("V4 court classification invalid")
    elif set(probes) == set(PROBE_KINDS) and classification != _court_classification(probes):
        errors.append("V4 court classification does not match receipts")
    if payload.get("successor_design_may_be_considered") is not False:
        errors.append("V4 cell cannot authorize successor before cross-cell court")
    return errors


def run_exp279_prebranch_observability_v4_court(
    *,
    root_seed: str,
    d_model: int,
    hidden_size: int,
    ...
    if not root_seed or not protocol_digest or not code_digest:
        raise ValueError("V4 root_seed, protocol_digest and code_digest are required")
    if min(
        d_model,
        hidden_size,
        target_parameters,
        ...
        raise ValueError("V4 counts and dimensions must be positive")
    if probe_folds != 3:
        raise ValueError("V4 court is frozen to three folds")
    if probe_replicates % (len(STRATA) * probe_folds) != 0:
        raise ValueError("V4 probe_replicates must balance all strata across folds")
    if constraints > variables or timesteps < constraints:
        raise ValueError("V4 world geometry is invalid")
    ...
    if noise_std < 0.0 or lr <= 0.0 or weight_decay < 0.0 or probe_lr <= 0.0:
        raise ValueError("V4 optimizer/noise parameters invalid")
    ...
    if not all(pair_audit.get(key) is True for key in required_audit):
        raise RuntimeError("V4 matched resource contract did not close")
    ...
    with torch.no_grad():
        for replicate in range(probe_replicates):
            ...
            batch = probe_generator.make_batch(
                ...
                rng_stream="augmentation",
                stratum=stratum,
                scope="synthetic-exp279-routing-development-prebranch-observability-v4-probe",
            )
            ...
            for key, value in features.items():
                feature_rows[key].append(value.detach().cpu())
    ...
    features = {key: torch.cat(rows, dim=0) for key, rows in feature_rows.items()}
    ...
    payload: dict[str, Any] = {
        ...
        "analysis_scope": "augmentation_only_frozen_prebranch_observability_direct_counterfactual_utility",
        ...
        "feature_boundary": {
            "branch_gru_used_for_probe_features": False,
            "branch_gru_used_for_counterfactual_labels": True,
            "event_projection_is_canonical_prebranch_shared_compute": True,
            "propagation_state_is_canonical_prebranch_compute": True,
            "allowed_temporal_summary": "last_projected_event_minus_first_projected_event",
            "allowed_features": [
                "propagation_state",
                "p_mean",
                "e_mean",
        ...
        "route_config": {
            ...
            "threshold_used_by_v4_court": False,
        ...
        "probe_config": {
            ...
            "fit_and_heldout_support_required": True,
            "raw_scores_compared_across_folds": False,
            "raw_scores_exported": False,
        ...
        "probe_dataset": {
            ...
            "natural_rescue_prevalence": pooled_counts["positive"] / pooled_counts["total"] if pooled_counts["total"] else 0.0,
            "fold_episode_counts": {
        ...
        "probe_economics": {
            ...
            "interpretation": "observability_upper_bound_only",
        },
        "economic_rule": {
            ...
            "pass_condition": "fit_and_heldout_support_closed_and_routed_verified_utility_strictly_greater_than_stop_verified_utility",
        ...
        "successor_design_may_be_considered": False,
        "cross_cell_decision_required": True,
        ...
        "analysis_boundary": (
            "augmentation-only DEVELOPMENT pre-branch observability upper-bound; "
            "cross-cell robustness required before successor design"
        ),
        "resource_pair_audit_digest": canonical_sha256(pair_audit),
        "artifact_digest": "",
    }
    payload["artifact_digest"] = _artifact_digest(payload)
    errors = validate_exp279_prebranch_observability_v4(payload)
    if errors:
        raise RuntimeError("invalid EXP-279 V4 pre-branch observability artifact: " + "; ".join(errors))
    return payload
