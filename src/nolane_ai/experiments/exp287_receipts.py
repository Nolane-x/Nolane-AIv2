...
def _validate_boundaries(receipt: dict[str, Any], *, context: str) -> None:
    for key in _BOUNDARY_FALSE_KEYS:
        _require(receipt.get(key) is False, f"EXP-287 {context} scientific boundary {key} must remain false")
...
def _validate_root_identity(receipt: dict[str, Any]) -> None:
    ...
    _require(receipt.get("experiment_id") == "EXP-287", "EXP-287 root experiment identity mismatch")
    ...
    _require(receipt.get("protocol_digest") == PROTOCOL_DIGEST, "EXP-287 protocol digest mismatch")
    _require(isinstance(receipt.get("model_init_seed"), int), "EXP-287 model init seed must be integer")
...
def _validate_training_evaluation_contract(receipt: dict[str, Any]) -> None:
    ...
    _require(training.get("rng_stream") == "augmentation", "EXP-287 training lineage must be augmentation")
    ...
    _require(training.get("loss_coefficients") == [1.0, 1.0, 1.0], "EXP-287 training loss weights drift")
    _require(evaluation.get("rng_stream") == "evaluation", "EXP-287 heldout lineage must be evaluation")
    ...
    _require(isinstance(batches, list) and len(batches) == 32, "EXP-287 heldout batch receipt count mismatch")
    ...
    for row in batches:
        ...
        _require(row.get("schema") == "NLM-EXP-287-SAME-WEIGHTS-BATCH-EVAL-V1", "EXP-287 heldout batch schema mismatch")
        _require(row.get("experiment_id") == "EXP-287", "EXP-287 heldout batch identity mismatch")
        _require(row.get("rng_stream") == "evaluation", "EXP-287 heldout batch stream mismatch")
        ...
        modes = row.get("modes")
        _require(isinstance(modes, dict) and set(modes) == _MODES, "EXP-287 heldout information-mode set mismatch")
    ...
    _require(observed_replicates == expected_replicates, "EXP-287 heldout replicate lineage drift")
...
def _validate_mode_audit(receipt: dict[str, Any]) -> None:
    ...
    _require(set(audit.get("modes", [])) == _MODES, "EXP-287 matched mode audit mode set mismatch")
    for key in (
        ...
        _require(audit.get(key) is True, f"EXP-287 matched resource audit {key} did not close")
    _require(audit.get("oracle_mode_deployable") is False, "EXP-287 oracle mode must remain non-deployable")
...
def _validate_metrics(receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    for key in ("control_solution_rate", "learned_solution_rate", "oracle_solution_rate", "top2_core_precision"):
        value = float(metrics[key])
        _require(0.0 <= value <= 1.0, f"EXP-287 {key} must be in [0,1]")
    _require(metrics["learned_oracle_information_delivered"] is False, "EXP-287 primitive metrics report learned oracle leakage")
    _require(metrics["precontradiction_conflict_delivery"] is False, "EXP-287 primitive metrics report precontradiction delivery")
    ...
    _require(localization.get("evaluated_variables") == 2048, "EXP-287 localization variable count mismatch")
    _require(localization.get("selected_core_members") == 512, "EXP-287 top-2 selection count mismatch")
    _require(
        float(localization.get("top2_core_precision")) == float(metrics["top2_core_precision"]),
        "EXP-287 localization precision and primitive metric disagree",
    )
    _require(float(localization.get("raw_core_prevalence")) == 0.25, "EXP-287 raw core prevalence drift")
    return metrics
...
def _validate_semantic_decision(receipt: dict[str, Any]) -> None:
    ...
    _require(receipt.get("decision") == expected, "EXP-287 forged root decision/classification")
...
def validate_exp287_root_receipt(receipt: dict[str, Any]) -> None:
    ...
    _require(receipt["artifact_digest"] == _artifact_digest(receipt), "EXP-287 root artifact digest mismatch")
...
def _validate_cross_identity(roots: list[dict[str, Any]]) -> list[dict[str, Any]]:
    _require(len(roots) == 4, "EXP-287 cross reducer requires exactly four canonical roots")
    for root in roots:
    ...
    _require(indices == [0, 1, 2, 3], "EXP-287 cross reducer has duplicate or missing canonical root")
    _require(
    ...
    _require(len(set(model_digests)) == 4, "EXP-287 canonical root model digests must be independently rooted")
    return ordered
...
def reduce_exp287_cross_root(roots: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {
        ...
        "root_classifications": {
            str(row["canonical_index"]): row["decision"]["classification"] for row in ordered
        },
        "root_decisions": {
            str(row["canonical_index"]): deepcopy(row["decision"]) for row in ordered
        },
        "decision": decision,
        "successor_design_authorized": authorized,
        "authorization_scope": "DESIGN_EXP288_BACKJUMP_COURT_ONLY" if authorized else "NONE",
...
def validate_exp287_cross_receipt(receipt: dict[str, Any]) -> None:
    ...
    _require(receipt.get("experiment_id") == "EXP-287", "EXP-287 cross experiment identity mismatch")
    _require(receipt.get("evidence_level") == "EV-E2", "EXP-287 cross evidence level must remain EV-E2")
    _require(
        receipt.get("analysis_scope") == "development_learned_conflict_localization_same_weights_oracle_value_capture",
        "EXP-287 cross analysis scope mismatch",
    )
    _require(receipt.get("protocol_digest") == PROTOCOL_DIGEST, "EXP-287 cross protocol digest mismatch")
    _require(receipt.get("geometry") == FROZEN_GEOMETRY, "EXP-287 cross frozen geometry mismatch")
    _require_hex64(receipt.get("geometry_digest"), "cross geometry_digest")
    _require(receipt.get("canonical_indices") == [0, 1, 2, 3], "EXP-287 cross canonical index set mismatch")
    ...
    _require(isinstance(classes, dict) and set(classes) == expected_keys, "EXP-287 cross root classification keys mismatch")
    _require(isinstance(decisions, dict) and set(decisions) == expected_keys, "EXP-287 cross root decision keys mismatch")
    _require(isinstance(artifact_digests, dict) and set(artifact_digests) == expected_keys, "EXP-287 cross root artifact keys mismatch")
    _require(isinstance(model_digests, dict) and set(model_digests) == expected_keys, "EXP-287 cross root model keys mismatch")
    for key in expected_keys:
        ...
        _require(isinstance(decision, dict), f"EXP-287 cross root decision {key} missing")
        _require(decision.get("classification") == classes[key], f"EXP-287 cross root decision/classification mismatch {key}")
        _require(classes[key] in {ESTABLISHED, NOT_ESTABLISHED, ORACLE_NOT_REPLICATED}, f"EXP-287 unknown root classification {key}")

    expected_decision = _cross_decision([str(classes[str(index)]) for index in range(4)])
    _require(receipt.get("decision") == expected_decision, "EXP-287 forged cross decision")
    expected_authorized = expected_decision == "LOCALIZATION_VALUE_RECURRENT"
    _require(receipt.get("successor_design_authorized") is expected_authorized, "EXP-287 cross authorization inconsistent with decision")
    _require(
    ...
    _require(receipt.get("mechanism_successor_authorized") is False, "EXP-287 cannot authorize a mechanism successor")
    _validate_boundaries(receipt, context="cross")
    _require_hex64(receipt.get("artifact_digest"), "cross artifact_digest")
    _require(receipt["artifact_digest"] == _artifact_digest(receipt), "EXP-287 cross artifact digest mismatch")
