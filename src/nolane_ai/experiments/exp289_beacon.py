...
def build_test_beacon_receipt(
    *,
    ...
    errors = validate_exp289_beacon_receipt(payload)
    if errors:
        raise ValueError("invalid synthetic EXP-289 beacon receipt: " + "; ".join(errors))
    return payload


def validate_exp289_beacon_receipt(
    payload: dict[str, Any],
    *,
    ...
    if not isinstance(payload, dict):
        return ["EXP-289 beacon receipt must be an object"]
    if payload.get("schema") != SCHEMA or payload.get("experiment_id") != EXPERIMENT_ID:
        errors.append("invalid EXP-289 beacon receipt identity")
    for field in ("source", "beacon_id", "evidence_reference"):
        if not isinstance(payload.get(field), str) or not payload.get(field):
            errors.append(f"EXP-289 beacon {field} missing")

    try:
        published = _parse_utc(payload.get("published_at_utc"))
    except ValueError:
        published = None
        errors.append("EXP-289 beacon publication timestamp invalid")

    if not _valid_entropy_hex(payload.get("entropy_hex")):
        errors.append("EXP-289 beacon entropy must contain at least 256 bits of hex entropy")
    ...
    if not isinstance(test_only, bool) or not isinstance(scientific, bool):
        errors.append("EXP-289 beacon evidence classification invalid")
    if test_only is True:
        ...
        errors.append("EXP-289 beacon test_only flag missing")
    ...
    if freeze_commit_timestamp_utc is not None:
        try:
            ...
            errors.append("EXP-289 freeze commit timestamp invalid")
        else:
...
def derive_exp289_challenge_seed(
    *,
    protocol_digest: str,
    ...
    material = (
        f"NLM|EXP-289|POST_FREEZE_CHALLENGE|{protocol_digest}|{freeze_commit_sha}|"
        f"{freeze_commit_timestamp_utc}|{beacon_receipt['receipt_digest']}|{stream}|{replicate}"
    )
