...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Classify the frozen EXP-279 V10 CRIC train60/train120 budget receipts"
    )
...
def _load_verified(path: Path) -> dict:
    ...
    if actual != expected:
        raise RuntimeError(f"V10 budget sidecar mismatch for {path}: expected {expected}, got {actual}")
    receipt = json.loads(payload.decode("utf-8"))
    if payload != canonical_receipt_bytes(receipt):
        raise RuntimeError(f"V10 budget receipt is not canonical JSON: {path}")
        ...
        json.dumps(
            {
                ...
                "authorized_representation_view": receipt["authorized_representation_view"],
