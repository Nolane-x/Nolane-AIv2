...
from nolane_ai.experiments.exp323_evidence import build_final_evidence


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Reduce frozen EXP-323 arm evidence")
...
def _load(path: str):
    ...
    if not isinstance(payload, dict):
        raise SystemExit(f"EXP-323 evidence is not an object: {path}")
    return payload
