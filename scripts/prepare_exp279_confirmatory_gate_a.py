...
def _reject_prefreeze_entropy_arguments(argv: Sequence[str]) -> None:
    forbidden = [
        ...
        if any(
            ...
            for flag in _FORBIDDEN_PREFREEZE_FLAGS
        )
    ]
    if forbidden:
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Prepare EXP-279 confirmatory Gate A transactionally. This command "
            "is strictly pre-beacon and cannot accept challenge entropy or a "
            "beacon receipt."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help=(
            "optional deterministic TEST-ONLY/replay override; when omitted, "
            "Gate A captures runtime UTC immediately after the trained "
...
def _exp279(protocol: dict[str, Any]) -> dict[str, Any]:
    for item in protocol.get("experiments") or []:
        if isinstance(item, dict) and item.get("experiment_id") == "EXP-279":
            return item
    raise RuntimeError("canonical Stage-A protocol does not contain EXP-279")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    # Fail before replaying training if any destination is ambiguous or already
    # occupied. Gate A is a single publication transaction, not six unrelated
    # writes.
    ...
    execution = _load_json(args.execution, label="EXP-279 DEVELOPMENT execution")
    registry = _load_json(args.registry, label="EXP-279 arm registry")

    _require_valid(
        "EXP-279 DEVELOPMENT execution",
        validate_exp279_paired_development(execution),
    )
    if execution.get("code_digest") != code_tree_digest:
        raise RuntimeError(
            "EXP-279 DEVELOPMENT execution code digest does not match the current "
            "Gate A source tree; regenerate the 32+ replicate DEVELOPMENT pilot "
            "from this exact source tree before sealing"
        )
    if execution.get("protocol_digest") != protocol_digest:
        raise RuntimeError("EXP-279 DEVELOPMENT execution protocol lineage mismatch")
    ...
    with tempfile.TemporaryDirectory(
        ...
        checkpoint_receipt = build_exp279_trained_checkpoint(
            execution_artifact=execution,
            checkpoint_path=checkpoint_stage,
        )
        _require_valid(
            "EXP-279 trained checkpoint receipt",
            validate_exp279_checkpoint_receipt(checkpoint_receipt),
        )
        if file_sha256(checkpoint_stage) != checkpoint_receipt.get(
            "checkpoint_file_sha256"
        ):
            raise RuntimeError(
                "EXP-279 staged checkpoint SHA256 does not match its receipt"
            )
        ...
        prep = build_exp279_confirmatory_prep(
            experiment=_exp279(protocol),
            execution_artifact=execution,
            arm_registry=registry,
            analysis_code_digest=code_tree_digest,
        )
        _require_valid(
            "EXP-279 confirmatory prep",
            validate_exp279_confirmatory_prep(prep),
        )
        if prep.get("status") != "CONFIRMATORY_GATE_A_PREPARED":
            raise RuntimeError(
                f"EXP-279 Gate A is not ready to seal: {prep.get('status')!r}"
            )
        ...
        _require_valid(
            "EXP-279 Gate A authorization",
            validate_exp279_gate_a_authorization(authorization),
        )

        seal = build_exp279_gate_a_seal(authorization=authorization)
        _require_valid(
            "EXP-279 Gate A seal",
            validate_exp279_gate_a_seal(seal),
        )
        ...
        _require_valid(
            "EXP-279 reconstruction authorization",
            validate_exp279_reconstruction_authorization(reconstruction),
        )

        # Recheck immediately before publication. A concurrent writer must not
        # produce a half-published ceremony or overwrite any existing evidence.
        _preflight_outputs(outputs)
        try:
        ...
        json.dumps(
            {
                ...
                "confirmatory_data_consumed": seal[
                ...
                "checkpoint_receipt_digest": checkpoint_receipt[
                    "receipt_digest"
                ],
