...
SCHEMA = "NLM-EXP-279-RESCUE-ANCHOR-PAIR-DEV-V1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run paired canonical-vs-rescue-anchor EXP-279 DEVELOPMENT control"
    )
    ...
    parser.add_argument(
        "--lineage-role",
        choices=("legacy_comparability", "fresh_primary"),
        required=True,
    )
...
def _evaluation_batch_digests(payload: dict) -> list[str]:
    return [
        str(row["paired_batch_digest"])
        for row in payload["evaluation"]["per_replicate"]
    ]
...
def main() -> int:
    ...
    intervention = run_exp279_rescue_anchor_replay_development(**common)
    ...
    if not all(control_identity.values()):
        ...
        raise RuntimeError("paired rescue-anchor control identity failed: " + ", ".join(failed))
    ...
    relative_utility_delta = (
        utility_delta / max(abs(baseline_utility), 1e-12)
    )
    ...
    comparison = {
        ...
        "lineage_role": args.lineage_role,
        "fresh_primary_single_use": args.lineage_role == "fresh_primary",
        ...
        "baseline": {
            ...
            "hybrid_relative_utility_gain_vs_best_simple": baseline_agg[
        ...
        "rescue_anchor": {
            ...
            "final_anchor_episodes": intervention["training"]["rescue_anchor_replay"][
                "final_anchor_episodes"
            ],
            "anchor_batches": intervention["training"]["rescue_anchor_replay"][
                "anchor_batches"
            ],
            ...
            "hybrid_relative_utility_gain_vs_best_simple": intervention_agg[
                "hybrid_relative_utility_gain"
            ],
            "best_simple_arm": intervention_agg["best_simple_arm"],
        },
        "delta_anchor_minus_baseline": {
        ...
        "analysis_boundary": (
            "DEVELOPMENT paired baseline-vs-intervention comparison only; fresh-primary lineage is single-use for this intervention and cannot promote EXP-279"
        ),
        "comparison_digest": "",
    }
    ...
    (args.output_dir / "rescue-anchor.json").write_text(
