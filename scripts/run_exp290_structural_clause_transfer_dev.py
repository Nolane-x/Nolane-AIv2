from __future__ import annotations

import argparse
import hashlib
import json
import os
from pathlib import Path
import subprocess
import sys
import tempfile
from typing import Any

ROOT = Path(__file__).resolve().parents[1]
SRC = ROOT / "src"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from nolane_ai.experiments.exp290_development_geometry import (
    load_exp290_development_geometry,
)
from nolane_ai.experiments.exp290_receipts import (
    canonical_receipt_bytes,
    seal_root_receipt,
    validate_root_receipt,
)
from nolane_ai.experiments.exp290_structural_clause_transfer import run_exp290_root
from nolane_ai.protocol.identity import (
    require_frozen_stage_a_v1_sha256,
    source_tree_digest,
)


GEOMETRY_MANIFEST = ROOT / "protocols" / "exp290_structural_clause_transfer_v1.json"
GEOMETRY_DIGEST = ROOT / "protocols" / "exp290_structural_clause_transfer_v1.sha256"
STAGE_A_DIGEST = ROOT / "protocols" / "stage_a_v1.sha256"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one frozen DEVELOPMENT-only EXP-290 canonical root"
    )
    parser.add_argument("--canonical-index", type=int, choices=(0, 1, 2, 3), required=True)
    parser.add_argument("--output", type=Path, required=True)
    return parser.parse_args()


def _repository_head() -> str:
    head = subprocess.check_output(
        ["git", "rev-parse", "HEAD"], cwd=ROOT, text=True
    ).strip()
    if len(head) != 40:
        raise RuntimeError("repository HEAD is not a full git SHA")
    int(head, 16)
    return head


def _write_temp(parent: Path, prefix: str, payload: bytes) -> Path:
    parent.mkdir(parents=True, exist_ok=True)
    fd, raw_path = tempfile.mkstemp(prefix=prefix, suffix=".tmp", dir=parent)
    path = Path(raw_path)
    try:
        with os.fdopen(fd, "wb") as handle:
            handle.write(payload)
            handle.flush()
            os.fsync(handle.fileno())
    except BaseException:
        path.unlink(missing_ok=True)
        raise
    return path


def _write_receipt_atomic(output: Path, receipt: dict[str, Any]) -> None:
    sidecar = output.with_name(output.name + ".sha256")
    if output.exists() or sidecar.exists():
        raise FileExistsError(f"output or sidecar already exists: {output}")

    payload = canonical_receipt_bytes(receipt)
    digest_payload = (hashlib.sha256(payload).hexdigest() + "\n").encode("ascii")
    receipt_tmp = _write_temp(output.parent, f".{output.name}.", payload)
    sidecar_tmp = _write_temp(output.parent, f".{sidecar.name}.", digest_payload)
    published_output = False
    try:
        os.link(receipt_tmp, output)
        published_output = True
        try:
            os.link(sidecar_tmp, sidecar)
        except BaseException:
            output.unlink(missing_ok=True)
            published_output = False
            raise
    except FileExistsError as exc:
        if published_output:
            output.unlink(missing_ok=True)
        raise FileExistsError(f"output or sidecar already exists: {output}") from exc
    finally:
        receipt_tmp.unlink(missing_ok=True)
        sidecar_tmp.unlink(missing_ok=True)


def main() -> int:
    args = parse_args()
    protocol_digest = STAGE_A_DIGEST.read_text(encoding="utf-8").strip()
    require_frozen_stage_a_v1_sha256(protocol_digest)
    geometry, geometry_digest = load_exp290_development_geometry(
        GEOMETRY_MANIFEST,
        GEOMETRY_DIGEST,
        protocol_digest=protocol_digest,
    )
    primitive = run_exp290_root(
        canonical_index=args.canonical_index,
        geometry=geometry,
        protocol_digest=protocol_digest,
        geometry_digest=geometry_digest,
        code_digest=source_tree_digest(ROOT),
    )
    primitive["repository_head"] = _repository_head()
    receipt = seal_root_receipt(primitive)
    validate_root_receipt(receipt)
    _write_receipt_atomic(args.output, receipt)
    print(
        json.dumps(
            {
                "artifact_digest": receipt["artifact_digest"],
                "canonical_index": receipt["canonical_index"],
                "decision": receipt["decision"],
                "code_digest": receipt["code_digest"],
                "geometry_digest": receipt["geometry_digest"],
                "output": str(args.output),
                "protocol_digest": receipt["protocol_digest"],
                "repository_head": receipt["repository_head"],
            },
            sort_keys=True,
        )
    )
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
