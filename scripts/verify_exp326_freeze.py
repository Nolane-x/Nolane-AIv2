...
from nolane_ai.experiments.exp326_identity import (
    Exp326ExecutionIdentity, source_tree_digest_from_git_tree_sha, validate_execution_identity,
)
...
def main(argv=None)->int:
    ...
    if set(payload)!={f.name for f in fields(Exp326ExecutionIdentity)}:
        raise SystemExit("EXP-326 marker identity field mismatch")
    identity=Exp326ExecutionIdentity(**payload)
    try: validate_execution_identity(identity)
    except ValueError as exc: raise SystemExit(str(exc)) from exc
    if identity.source_commit_sha!=a.source_commit_sha:
        raise SystemExit("EXP-326 marker source commit mismatch")
    if Path(a.marker_json).read_bytes()!=marker_json_bytes(identity):
        raise SystemExit("EXP-326 marker JSON is not canonical")
    if Path(a.marker_sha256).read_bytes()!=marker_sidecar_bytes(identity):
        raise SystemExit("EXP-326 marker sidecar mismatch")
    ...
    if identity.source_tree_digest!=source_tree_digest_from_git_tree_sha(tree):
        raise SystemExit("EXP-326 source tree digest mismatch")
    ...
    if hashlib.sha256(workflow).hexdigest()!=identity.workflow_sha256:
        raise SystemExit("EXP-326 workflow digest mismatch")
    if git(root,"merge-base",BASE_SHA,a.source_commit_sha)!=BASE_SHA:
        raise SystemExit("EXP-326 source is not descended from sealed EXP-325 marker")
    ...
    if git(root,"rev-parse","HEAD^")!=a.source_commit_sha:
        raise SystemExit("EXP-326 marker parent mismatch")
    marker_changed=tuple(
        x for x in git(root,"diff-tree","--no-commit-id","--name-only","-r","HEAD").splitlines() if x
    )
