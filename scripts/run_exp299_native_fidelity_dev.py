...
GEOMETRY_MANIFEST = ROOT / "protocols" / "exp299_native_fidelity_geometry_v1.json"
GEOMETRY_DIGEST = ROOT / "protocols" / "exp299_native_fidelity_geometry_v1.sha256"
GEOMETRY_SCHEMA = "NLM-EXP-299-NATIVE-FIDELITY-GEOMETRY-V1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run frozen DEVELOPMENT-only EXP-299 root or four-root cross reduction"
    )
    subparsers = parser.add_subparsers(dest="mode", required=True)

    root = subparsers.add_parser("root", help="run one canonical EXP-299 root")
    root.add_argument("--canonical-index", type=int, choices=(0, 1, 2, 3), required=True)
    root.add_argument("--output", type=Path, required=True)

    cross = subparsers.add_parser("cross", help="reduce exactly four canonical EXP-299 roots")
...
def _load_frozen_geometry() -> tuple[dict[str, Any], str]:
    ...
    if not isinstance(payload, dict):
        raise ValueError("EXP-299 frozen geometry manifest must be an object")
    ...
    if observed_digest != declared_digest:
        raise ValueError("EXP-299 frozen geometry digest mismatch")
    ...
    if payload.get("schema") != GEOMETRY_SCHEMA:
        raise ValueError("EXP-299 frozen geometry schema mismatch")
    if payload.get("experiment_id") != "EXP-299":
        raise ValueError("EXP-299 frozen geometry experiment mismatch")
    if payload.get("authority_scope") != "DEVELOPMENT_EV_E2_ONLY":
        raise ValueError("EXP-299 frozen geometry authority scope mismatch")
    if payload.get("canonical_indices") != [0, 1, 2, 3]:
        raise ValueError("EXP-299 frozen geometry canonical roots mismatch")
    if payload.get("protocol_digest") != protocol_digest:
        raise ValueError("EXP-299 frozen geometry protocol digest mismatch")
    if payload.get("domains") != [
        "code_invariant",
        "causal_diagnosis",
        "grounded_language_ambiguity",
    ]:
        raise ValueError("EXP-299 frozen geometry domain identity mismatch")
    expected_scalars = {
        "fit_replicates": 96,
    ...
    for key, value in expected_scalars.items():
        if payload.get(key) != value:
            raise ValueError(f"EXP-299 frozen geometry mismatch: {key}")
    if payload.get("heldout_generalization_scope") != "instance_lineage_within_frozen_domain_family":
        raise ValueError("EXP-299 held-out generalization scope mismatch")
    if payload.get("successor_scope_if_recurrent") != "DESIGN_EXP300_INTEGRATED_COURT_ONLY":
        raise ValueError("EXP-299 frozen successor scope mismatch")
    for flag in (
        ...
        if payload.get(flag) is not False:
            raise ValueError(f"EXP-299 forbidden frozen geometry flag enabled: {flag}")
...
def _load_root(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise ValueError(f"EXP-299 root receipt must be an object: {path}")
    errors = validate_exp299_root(payload)
    if errors:
        raise ValueError(f"invalid EXP-299 root receipt {path}: {'; '.join(errors)}")
    if payload.get("geometry_digest") != geometry_digest:
        raise ValueError(f"EXP-299 root geometry digest mismatch: {path}")
    if payload.get("protocol_digest") != geometry["protocol_digest"]:
        raise ValueError(f"EXP-299 root protocol digest mismatch: {path}")
    sidecar = path.with_name(path.name + ".sha256")
    if sidecar.exists():
        expected = hashlib.sha256(path.read_bytes()).hexdigest()
        observed = sidecar.read_text(encoding="ascii").strip()
        if observed != expected:
            raise ValueError(f"EXP-299 root sidecar mismatch: {path}")
    return payload
...
def _run_root(canonical_index: int) -> dict[str, Any]:
    ...
    return run_exp299_root(
        ...
        fit_replicates=int(geometry["fit_replicates"]),
        fit_start_replicate=int(geometry["fit_start_replicate"]),
        ...
        gradient_clip_norm=float(geometry["gradient_clip_norm"]),
        authority_threshold=float(geometry["authority_threshold"]),
...
def main() -> int:
    ...
    if args.mode == "root":
        ...
        errors = validate_exp299_root(receipt)
        if errors:
            raise RuntimeError("EXP-299 root validation failed: " + "; ".join(errors))
    else:
        from nolane_ai.experiments.exp299_cross_reducer import (
            reduce_exp299_roots,
            validate_exp299_cross,
        )

        roots = [_load_root(path) for path in args.root_receipt]
        receipt = reduce_exp299_roots(roots)
        errors = validate_exp299_cross(receipt)
        if errors:
            raise RuntimeError("EXP-299 cross validation failed: " + "; ".join(errors))
