...
from nolane_ai.experiments.exp325_contract import FAMILIES, canonical_json_bytes
from nolane_ai.experiments.exp325_identity import Exp325ExecutionIdentity, validate_execution_identity
from nolane_ai.experiments.exp325_runtime import run_family_worlds
...
def main(argv=None)->int:
    ...
    if set(raw)!={f.name for f in fields(Exp325ExecutionIdentity)}:
        raise SystemExit("EXP-325 identity fields mismatch")
    identity=Exp325ExecutionIdentity(**raw); validate_execution_identity(identity)
    payload=run_family_worlds(
