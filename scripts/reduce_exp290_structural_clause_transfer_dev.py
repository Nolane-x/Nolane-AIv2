...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Reduce exactly four frozen EXP-290 DEVELOPMENT root receipts"
    )
...
def _load_root_receipt(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(parsed, dict):
        raise ValueError(f"EXP-290 root receipt must be a JSON object: {path}")
    canonical = canonical_receipt_bytes(parsed)
    if raw != canonical:
        raise ValueError(f"EXP-290 root receipt is not canonical JSON: {path}")
    sidecar = path.with_name(path.name + ".sha256")
    if not sidecar.exists():
        raise ValueError(f"EXP-290 root receipt sidecar is missing: {sidecar}")
    expected = sidecar.read_text(encoding="utf-8")
    actual = hashlib.sha256(raw).hexdigest() + "\n"
    if expected != actual:
        raise ValueError(f"EXP-290 root receipt sidecar mismatch: {path}")
    validate_root_receipt(parsed)
    return parsed
...
def main() -> int:
    ...
    receipt = reduce_cross_roots(roots)
