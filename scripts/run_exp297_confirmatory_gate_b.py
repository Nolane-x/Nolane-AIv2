...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Execute the frozen EXP-297 future-beacon Gate B path. Challenge seeds "
            "are derived internally from the sealed lineage and beacon receipt."
        )
    )
    parser.add_argument(
        "--protocol",
    ...
    parser.add_argument(
        "--test-only",
        action="store_true",
        help="require a TEST-ONLY synthetic beacon and forbid scientific promotion",
    )
...
def _enforce_beacon_mode(beacon: dict[str, Any], *, test_only: bool) -> None:
    beacon_test_only = beacon.get("test_only") is True
    if test_only:
        if not beacon_test_only:
            raise RuntimeError("--test-only requires a TEST-ONLY synthetic beacon receipt")
        if beacon.get("scientific_evidence_eligible") is not False:
            raise RuntimeError("TEST-ONLY beacon cannot be scientific evidence")
        return
    if beacon_test_only:
        raise RuntimeError(
            "synthetic beacon requires explicit --test-only; refusing accidental "
            "execution outside the TEST-ONLY lane"
        )
    if beacon.get("scientific_evidence_eligible") is not True:
        raise RuntimeError(
            "non-test Gate B requires a beacon receipt marked scientific_evidence_eligible"
        )


def _enforce_test_only_outputs(raw: dict[str, Any], analysis: dict[str, Any]) -> None:
    if raw.get("test_only") is not True or raw.get("scientific_evidence_eligible") is not False:
        raise RuntimeError("TEST-ONLY raw artifact crossed the scientific evidence boundary")
    if raw.get("confirmatory_data_consumed") is not False:
        raise RuntimeError("TEST-ONLY raw artifact consumed confirmatory data")
    if analysis.get("evidence_level") != "EV-E2" or analysis.get("decision") != "UNVERIFIED":
        raise RuntimeError("TEST-ONLY analysis attempted scientific promotion")
    if analysis.get("scientific_evidence_eligible") is not False:
        raise RuntimeError("TEST-ONLY analysis became scientific evidence")
    if analysis.get("confirmatory_data_consumed") is not False:
        raise RuntimeError("TEST-ONLY analysis consumed confirmatory data")
    if analysis.get("decision_rule_executed") is not False:
        raise RuntimeError("TEST-ONLY analysis executed the scientific decision rule")
    if analysis.get("semantic_authority_promoted") is not False:
        raise RuntimeError("TEST-ONLY analysis promoted semantic authority")
...
def main() -> int:
    ...
    seal = _load_json(args.seal, label="EXP-297 Gate A seal")
    _require_valid("EXP-297 Gate A seal", validate_exp297_confirmatory_gate_a_seal(seal))

    if (seal.get("lineage") or {}).get("protocol_digest") != protocol_digest:
        raise RuntimeError("EXP-297 Gate B protocol/seal lineage mismatch")
    if seal.get("code_tree_digest") != code_tree_digest:
        raise RuntimeError(
            "EXP-297 Gate A seal code tree no longer matches the current source tree; "
            "production-code drift requires a new Gate A freeze before Gate B"
        )

    beacon = _load_json(args.beacon, label="EXP-297 public beacon receipt")
    beacon_errors = validate_exp297_beacon_receipt(
        beacon,
        freeze_commit_timestamp_utc=seal.get("freeze_commit_timestamp_utc"),
    )
    if beacon_errors:
        raise RuntimeError("invalid EXP-297 public beacon receipt: " + "; ".join(beacon_errors))
    _enforce_beacon_mode(beacon, test_only=args.test_only)

    reconstruction = seal.get("reconstruction_authorization")
    if not isinstance(reconstruction, dict):
        raise RuntimeError("EXP-297 Gate A seal reconstruction authorization missing")

    raw = execute_exp297_confirmatory_challenge(
        reconstruction_authorization=reconstruction,
        beacon_receipt=beacon,
        freeze_commit_sha=seal["freeze_commit_sha"],
        freeze_commit_timestamp_utc=seal["freeze_commit_timestamp_utc"],
        executor_code_digest=code_tree_digest,
    )
    _require_valid("EXP-297 confirmatory raw artifact", validate_exp297_confirmatory_raw(raw))
    ...
    _require_valid(
        "EXP-297 confirmatory analysis",
        validate_exp297_confirmatory_analysis(analysis),
    )

    if args.test_only:
        _enforce_test_only_outputs(raw, analysis)
        ...
        json.dumps(
            {
                ...
                "challenge_materialized": analysis["challenge_materialized"],
                "decision_rule_executed": analysis["decision_rule_executed"],
                "semantic_authority_promoted": analysis["semantic_authority_promoted"],
