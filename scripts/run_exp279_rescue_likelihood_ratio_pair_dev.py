...
from nolane_ai.experiments.exp279_paired_runner import PRIMARY_METRIC, run_exp279_paired_development
from nolane_ai.experiments.exp279_rescue_likelihood_ratio import run_exp279_rescue_likelihood_ratio_development
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--root-seed", default="20260911-exp279-rescue-likelihood-ratio-dev")
...
def main() -> int:
    ...
    if args.eval_start_replicate != PREREGISTERED_EVAL_START or args.eval_replicates != PREREGISTERED_EVAL_REPLICATES:
        raise SystemExit("fresh single-use DEVELOPMENT lineage is frozen at 50000..50032")
    ...
    intervention = run_exp279_rescue_likelihood_ratio_development(**common)

    identity = {
        "initial_state_match": baseline["initial_state"] == intervention["initial_state"],
        "training_batch_lineage_match": baseline["training"]["paired_batch_digests"] == intervention["training"]["paired_batch_digests"],
        "evaluation_batch_lineage_match": _evaluation_batch_digests(baseline) == _evaluation_batch_digests(intervention),
        "pair_audit_digest_match": baseline["resource_match"]["pair_audit_digest"] == intervention["resource_match"]["pair_audit_digest"],
        "propagation_final_state_match": baseline["final_state"]["propagation_only_digest"] == intervention["final_state"]["propagation_only_digest"],
        "branch_final_state_match": baseline["final_state"]["branch_only_digest"] == intervention["final_state"]["branch_only_digest"],
    }
    if not all(identity.values()):
        failed = [key for key, value in identity.items() if not value]
        raise RuntimeError("paired likelihood-ratio identity failed: " + ", ".join(failed))
    ...
    comparison = {
        ...
        "analysis_boundary": "single-use DEVELOPMENT paired court only; cannot promote EXP-279",
        "comparison_digest": "",
    }
    comparison["comparison_digest"] = canonical_sha256({k: v for k, v in comparison.items() if k != "comparison_digest"})

    args.output_dir.mkdir(parents=True, exist_ok=False)
    for name, payload in (("baseline.json", baseline), ("likelihood-ratio.json", intervention), ("comparison.json", comparison)):
        (args.output_dir / name).write_text(json.dumps(payload, sort_keys=True, indent=2) + "\n", encoding="utf-8")
