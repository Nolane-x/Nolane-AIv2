...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run one frozen EXP-319 Stage A training chunk")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.chunk_index not in range(4):
        raise SystemExit("Stage A chunk-index must be in 0..3")
    chunk = CONTRACT.stage_a.max_chunk_steps
    start = args.chunk_index * chunk
    end = min(start + chunk, max(CONTRACT.stage_a.checkpoints))
    ...
    if args.chunk_index and (not parent_checkpoint or not parent_receipt):
        raise SystemExit("continuation Stage A chunk requires parent checkpoint and receipt")
