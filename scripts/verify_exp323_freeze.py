...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Verify sealed EXP-323 execution identity")
...
def main(argv: list[str] | None = None) -> int:
    ...
    expected_fields = {item.name for item in fields(Exp323ExecutionIdentity)}
    if set(payload) != expected_fields:
        raise SystemExit("EXP-323 marker identity field mismatch")
    identity = Exp323ExecutionIdentity(**payload)
    try:
    ...
    if identity.source_commit_sha != args.source_commit_sha:
        raise SystemExit("EXP-323 marker source commit mismatch")
    if Path(args.marker_json).read_bytes() != canonical_marker_json_bytes(identity):
        raise SystemExit("EXP-323 marker JSON is not canonical")
    if Path(args.marker_sha256).read_bytes() != marker_sidecar_bytes(identity):
        raise SystemExit("EXP-323 marker SHA sidecar mismatch")
    ...
    if identity.source_tree_digest != source_tree_digest_from_git_tree_sha(tree_sha):
        raise SystemExit("EXP-323 source tree digest mismatch")

    workflow = _git_bytes(repo, "show", f"{args.source_commit_sha}:{EXP323_WORKFLOW_PATH}")
    if hashlib.sha256(workflow).hexdigest() != identity.workflow_sha256:
        raise SystemExit("EXP-323 sealed workflow digest mismatch")

    merge_base = _git(repo, "merge-base", EXP323_BASE_SHA, args.source_commit_sha)
    if merge_base != EXP323_BASE_SHA:
        raise SystemExit("EXP-323 source is not descended from sealed EXP-322 marker")

    changed = tuple(
        line
        for line in _git(
            repo, "diff", "--name-only", EXP323_BASE_SHA, args.source_commit_sha
        ).splitlines()
        if line
    )
    ...
    if parent != args.source_commit_sha:
        raise SystemExit("EXP-323 marker parent is not the sealed source commit")
