...
SCHEMA = "NLM-EXP-279-PRIOR-CORRECTED-RESCUE-PAIR-DEV-V1"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run paired canonical-vs-prior-corrected-rescue EXP-279 DEVELOPMENT control"
    )
    ...
    parser.add_argument("--eval-start-replicate", type=int, required=True)
...
def main() -> int:
    ...
    if args.eval_start_replicate != 40_000:
        raise SystemExit("this preregistered intervention requires eval_start_replicate=40000")
    if args.eval_replicates != 33:
        raise SystemExit("this preregistered intervention requires exactly 33 evaluation replicates")
    ...
    intervention = run_exp279_prior_corrected_rescue_development(**common)
    ...
    if not all(control_identity.values()):
        ...
        raise RuntimeError("paired prior-corrected rescue control identity failed: " + ", ".join(failed))
    ...
    comparison = {
        ...
        "prior_corrected": {
            ...
            "final_anchor_episodes": replay["final_anchor_episodes"],
            "anchor_batches": replay["anchor_batches"],
            "natural_positive_count": replay["natural_positive_count"],
            "natural_total_count": replay["natural_total_count"],
            "natural_positive_fraction": replay["natural_positive_fraction"],
            "incremental_cost_ratio": replay["incremental_cost_ratio"],
        ...
        "delta_prior_corrected_minus_baseline": {
        ...
        "analysis_boundary": (
            "single-use DEVELOPMENT paired baseline-vs-prior-corrected-rescue comparison on preregistered evaluation IDs 40000..40032; cannot promote EXP-279"
        ),
        "comparison_digest": "",
    }
    ...
    (args.output_dir / "prior-corrected.json").write_text(
