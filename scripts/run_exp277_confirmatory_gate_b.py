...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Execute the frozen EXP-277 Gate B path from an explicitly supplied "
            "post-freeze beacon receipt. This command never fetches beacon data "
            "from the network and exposes no raw challenge seed."
        )
    )
    parser.add_argument(
        "--protocol",
...
def _sealed_executor_digest(seal: dict[str, Any]) -> str:
    ...
    if not isinstance(authorization, dict):
        raise RuntimeError("EXP-277 Gate A seal authorization snapshot missing")
    machinery = authorization.get("machinery_digests")
    if not isinstance(machinery, dict):
        raise RuntimeError("EXP-277 Gate A machinery digest snapshot missing")
    digest = machinery.get("executor_code_digest")
    if not isinstance(digest, str) or len(digest) != 64:
        raise RuntimeError("EXP-277 frozen executor code digest missing")
    return digest
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    seal = _load_json(args.seal, label="EXP-277 Gate A seal")
    reconstruction = _load_json(
        args.reconstruction,
        label="EXP-277 reconstruction authorization",
    )
    beacon = _load_json(args.beacon, label="EXP-277 public beacon receipt")
    checkpoint_receipt = _load_json(
        args.checkpoint_receipt,
        label="EXP-277 checkpoint receipt",
    )
    ...
    if authorization.get("protocol_digest") != protocol_digest:
        raise RuntimeError("EXP-277 Gate B protocol/seal lineage mismatch")
    if reconstruction.get("protocol_digest") != protocol_digest:
        raise RuntimeError("EXP-277 Gate B protocol/reconstruction lineage mismatch")
    ...
    if ceremony_errors:
        raise RuntimeError(
            "invalid EXP-277 Gate B ceremony: " + "; ".join(ceremony_errors)
        )
