...
from nolane_ai.experiments.exp336_artifact_audit import audit_chunk_chain, audit_final_evidence


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Independently verify EXP-336 immutable chunk artifact ZIPs."
    )
    parser.add_argument("--execution-identity", required=True)
    parser.add_argument("--chunk-zip", action="append", required=True)
    parser.add_argument(
        "--final-json",
        help="Optional EXP-336 final evidence JSON; requires a complete 8-chunk chain.",
    )
    ...
    if not isinstance(identity, dict):
        raise SystemExit("EXP-336 execution identity must be a JSON object")
